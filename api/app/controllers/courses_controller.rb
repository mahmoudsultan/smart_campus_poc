class CoursesController < ApplicationController
  def show_lectures
    ...
    #   format.json do
        ...
        @q = @q.joins("INNER JOIN `courses` ON `courses`.`id` = `course_offerings`.`course_id` INNER JOIN `groups` ON "\
          "`groups`.`course_offering_id` = `course_offerings`.`id` "\
          "INNER JOIN `group_users` ON `group_users`.`group_id` = `groups`.`id` INNER JOIN `users` ON `users`.`id` = `group_users`.`user_id`"\
          "INNER JOIN `lectures` ON `lectures`.`course_offering_id` = `course_offerings`.`id`"\
          "INNER JOIN `klasses` ON `klasses`.`id` = `lectures`.`klass_id`").where(
            "`users`.`id`=? AND `users`.`role`='professor' AND `course_offerings`.`term`=? AND `course_offerings`.`year`=?", params[:pid], params[:term], params[:year]
          ).select('users.name,courses.code,courses.title,klasses.name,klasses.floor,klasses.capacity')
